...
AcceptAnswer:
	...
	Public Sub ScriptTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ScriptTimer.Tick
		...
		'Debug.Print("ScriptTick = " & ssh.ScriptTick)
...
CancelGoto:
	...
	Public Sub TypingDelay()
		...
		End If
		If ssh.ContactRoomUpdate <> "" Then
			ChatAddSystemMessage(ssh.ContactRoomUpdate, False)
			ssh.ContactRoomUpdate = ""
		End If
		ssh.TypeDelay = ssh.StringLength
		If FrmSettings.typeinstantlyCheckBox.Checked = True Then ssh.TypeDelay = 0
	...
	Public Sub TypingDelayGeneric()
		...
		ssh.TypeDelay = ssh.StringLength
		If FrmSettings.typeinstantlyCheckBox.Checked = True Then ssh.TypeDelay = 0
		If ssh.RapidCode = True Then ssh.TypeDelay = 0
...
SkipIsTyping:
				...
				If FrmSettings.typeinstantlyCheckBox.Checked = True Then ssh.StringLength = 0
				If ssh.RapidCode = True Then ssh.StringLength = 0
...
DommeSlideshowFallback:
				...
				If ssh.RLGLGame = True And ssh.RedLight = False Then
					If (DomVideo.State = VideoHandler.PlayState.Paused) Then
                        DomVideo.PlayIt()
				...
				If ssh.RLGLGame = True And ssh.RedLight = True Then
					If (DomVideo.State = VideoHandler.PlayState.Playing) Then
                        DomVideo.PauseIt()
						ssh.SubStroking = False
						StrokePace = 0
				...
				If ssh.YesOrNo = False Then
					If ssh.RapidCode = True Then
						If WaitTimer.Enabled = False Then
							RunFileText()
						Else
							WaitTimer.Stop()
							ssh.ScriptTick = ssh.WaitTick
							ssh.WaitTick = 0
							ScriptTimer.Start()
						End If
					Else
...
#Region " VLC "
	...
	Private Sub BTNLoadVideo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNLoadVideo.Click
		...
		If (OpenFileDialog2.ShowDialog = Windows.Forms.DialogResult.OK) Then
            ...
            DomVideo.Visible = True
            DomVideo.stretchToFit = True
            ...
            DomVideo.MediaURI = OpenFileDialog2.FileName
	...
	Private Sub BTNVideoControls_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNVideoControls.Click
		...
		If DomVideo.ShowControls Then
            DomVideo.ShowControls = False
...
GetAnotherRandomVideo:
		...
		If ssh.JumpVideo = True Then
			Do
				Application.DoEvents()
            Loop Until (DomVideo.State = VideoHandler.PlayState.Playing)
	...
	Friend Sub PlayRandomJOI()
        ...
        DomVideo.MediaURI = JOIVideos(JOIVideoLine)
	...
	Friend Sub PlayRandomCH()
        ...
        DomVideo.MediaURI = CHVideos(CHVideoLine)
...
CensorConstant:
				...
				Try
                    CensorshipBarY2 = ssh.randomizer.Next(200, DomVideo.Height / 2)
				Catch
					CensorshipBarY2 = 100
				End Try
                ...
                'QnD-BUGFIX: if CensorshipBar.Width > DomVideo.Width then ArgumentOutOfRangeException 
                CensorshipBarX = ssh.randomizer.Next(5, If(CensorshipBar.Width > DomVideo.Width, DomVideo.Width, DomVideo.Width - CensorshipBar.Width + 1))
                CensorshipBarY = ssh.randomizer.Next(5, If(CensorshipBar.Height > DomVideo.Height, DomVideo.Height, DomVideo.Height - CensorshipBar.Height + 1))
...
StatusUpdateEnd:
	...
	Private Sub UpdatesTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UpdatesTimer.Tick
...
ExternalAudio:
		...
		If StringClean.Contains("@JumpVideo") Then
			...
			If (DomVideo.State = VideoHandler.PlayState.Playing) Then
                Dim VideoLength As Integer = DomVideo.CurrentMediaDuration
                ...
                DomVideo.CurrentMediaCurrentPosition = VideoLength - VidPoint
...
VTSkip:
		...
		If StringClean.Contains("@RiskyPayout") Then
			If FrmSettings.CBGameSounds.Checked = True And File.Exists(Application.StartupPath & "\Audio\System\PayoutSmall.wav") Then
                FrmCardList.GameVideo.Looped = False
                FrmCardList.GameVideo.Volume = 20
                FrmCardList.GameVideo.MediaURI = Application.StartupPath & "\Audio\System\PayoutSmall.wav"
			End If
...
#End Region	' Flags/Dates/Variables
	...
	Public Function GetLocalImage(ByVal LocTag As String, Optional ByVal isDommeTag As Boolean = False, Optional ByVal selectionType As String = "All") As String
			...
			For Each m As Match In mc
				...
				ElseIf __ConditionDic.Keys.Contains(m.Value) = False Then
					...
					Select Case m.Value.ToUpper
						...
						Case "@ImageTagOr(".ToUpper : Condition = GetLocalImage(FilterString, , "Or")
						Case "@ImageTagAny(".ToUpper : Condition = GetLocalImage(FilterString, , "Any")
						Case Else
							'<= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <= <=
...
#Region "---------------------------------------------------- Domme-Video -------------------------------------------------------"
	...
	Private Sub DomVideo_PlayStateChange(ByRef sender As Object, ByVal e As VideoHandler.PlayState) Handles DomVideo.StateChanged


		If (e = VideoHandler.PlayState.Playing) Then
		...
		If (e = VideoHandler.PlayState.Stopped) Then
            'Debug.Print("Video Stopped Called")
	...
	Private Sub VideoTimer2_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles VideoTimer2.Tick


		If DomVideo.CurrentPlayListCount <> 0 Then
			Try
                Dim VideoLength As Integer = DomVideo.CurrentMediaDuration
                Dim VideoRemaining As Integer = Math.Floor(DomVideo.CurrentMediaDuration - DomVideo.CurrentMediaCurrentPosition)
        ...
        If ssh.DomTypeCheck = True Or DomVideo.State = VideoHandler.PlayState.Stopped Or DomVideo.State = VideoHandler.PlayState.Paused Then Return

        'Debug.Print("New movie loaded: " & DomVideo.URL.ToString)

        ssh.VidFile = Path.GetFileName(DomVideo.MediaURI.ToString)
		...
		If File.Exists(Application.StartupPath & "\Scripts\" & dompersonalitycombobox.Text & "\Video\Scripts\" & ssh.VidFile & ".txt") Then
            ...
            Dim VideoPos As Integer = Math.Ceiling(DomVideo.CurrentMediaCurrentPosition)
...
#End Region 'Domme-Video
	...
	Private Sub WaitTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles WaitTimer.Tick
		...
		'Debug.Print("WaitTick = " & ssh.WaitTick)
...
restartInstantly:
	...
	Private Sub updateGroup(ByVal contact As String)
		Dim name As String
		Select Case contact
			Case 1
		...
		If Not ssh.Group.Contains(contact) Then
			...
			ssh.ContactRoomUpdate = name & " has joined the Chat room"
		Else
			ssh.Group = ssh.Group.Replace(contact, "")
			'ChatAddSystemMessage(name & " has left the Chat room", False)
			ssh.ContactRoomUpdate = name & " has left the Chat room"
		End If
		If ssh.Group = "D" Then
	...
	Private Sub UpdateStageTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UpdateStageTimer.Tick
		If FrmSettings.CBSettingsPause.Checked = True And FrmSettings.SettingsPanel.Visible = True Then Return
		...
		Try
			...
			If My.Computer.Keyboard.CtrlKeyDown Then
				Dim fsd As New SaveFileDialog With {.Filter = "Saved Session|*" & Path.GetExtension(SavedSessionDefaultPath) & "",
				   ...
				   .Title = "Select a destination to save the session to.",
...
#Region "-------------------------------------------------- Randomizer-App ----------------------------------------------------"
	...
	Private Sub VideoJump2Random(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button12.Click
		Try
            If DomVideo.CurrentPlayListCount = 0 Then Throw New Exception("No Video playing - can't jump.")
...
#Region "------------------------------------------------- Hypno-Guide App ----------------------------------------------------"
	...
	Private Sub BTNHypnoGenStart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNHypnoGenStart.Click
		...
		If ssh.HypnoGen = False Then
            ...
            If File.Exists(HypnoTrack) Then DomVideo.MediaURI = HypnoTrack
			ssh.HypnoGen = True
			ssh.AFK = True
			ssh.SaidHello = True
...
#End Region	' Apps
	...
	Private Sub selectVideo(ByVal StringClean As String)
		...
		If VideoClean.Contains("*") Then
			...
			If VideoList.Count > 0 Then
                DomVideo.MediaURI = VideoList(ssh.randomizer.Next(0, VideoList.Count))
                DomVideo.Visible = True
				mainPictureBox.Visible = False
	...
	Private Function checkSubAnswer(Optional ByVal caseToCheck As String = "", Optional ByVal reduceErrors As Boolean = True) As Integer
		ssh.DomChat = ""
